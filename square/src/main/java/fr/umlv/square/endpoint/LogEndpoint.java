...
public class LogEndpoint {
	...
	/**
	 * Filter the data ArrayList with only the logs that are before the number of
	 * minutes given as argument
	 * 
	...
	private static Stream<LogTimeResponse> logsFiltedByTime(String timestamp) {
		...
		return data.stream().filter((log) -> log.getTimestamp().compareTo(filterTimestamp.toString()) > 0);
	...
	/**
	 * Allow to know if a String is a numeric. Uses to define if the user want to
	 * filter logs by an id
	 * 
