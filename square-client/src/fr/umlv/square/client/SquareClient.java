	...
	public void sendLog() throws IOException, InterruptedException {
		...
		while (true) {
			new Thread(() -> {
				...
				map.entrySet().forEach((elt) -> builder.add("\"" + elt.getKey() + "\":\"" + elt.getValue() + "\""));

				System.out.println(builder.toString());
				var request = HttpRequest.newBuilder().uri(URI.create(uri)).header("Content-Type", "application/json")
